...
- параноидальная, но не искажающая смысл: убирает управляющие символы (кроме `\n`/`\r`/`\t`) и HTML/script-теги (XSS), схлопывает только горизонтальные пробелы — переносы строк сохраняются, т.к. в письмо должен уйти комментарий ровно как пришло после санитации, но в любом случае - через фильтры безопасного вывода
...
- **важный принцип, действует везде, где данные обращения выводятся** (письмо, лог и т.п.): при конфликте между «текстовая точность вывода» и «безопасность вывода» приоритет всегда за безопасностью — только безопасные (экранирующие) механизмы вывода, никогда «сырой»/неэкранированный вывод ради точности (пример: `NewFeedbackMail`, см. `SPECS-Mail.md` — используется `{{ }}`, а не `{!! !!}`).
